...
def show_ai_model():
    ...
    st.info("Burada yapay zeka modelleriyle tahminleme ve optimizasyon çalışmaları yer alacaktır.")

def show_energy_efficiency():
    st.markdown("## ⚙️ 3. Enerji Verimliliği")
    st.info("Enerji verimliliği analizleri ve önerileri burada sunulacaktır.")

def main():
    st.sidebar.title("Sayfalar")
    menu = {
        "1. Veri Girişi": show_data_entry,
        "2. AI Model": show_ai_model,
        "3. Enerji Verimliliği": show_energy_efficiency
    }
    selected = st.sidebar.radio("Menü", list(menu.keys()))
    menu[selected]()
