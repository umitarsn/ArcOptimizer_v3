...
def show_setup_form():
    ...
    st.markdown(
        ...
        "3. Detay için ℹ️ simgesine tıklayınız."
    )
    ...
    pct_required = round(100 * required_filled / required_fields, 1) if required_fields else 0.0
...
def show_runtime_page(sim_mode: bool):
    ...
    if sim_mode:
        st.info("🧪 **Simülasyon Modu Aktif.** Gösterilen veriler simülasyondur, yeni girişler kalıcı kaydedilmez.")

    st.markdown(
        "Her **şarj / heat** için toplanan operasyonel verileri buraya girin "
        "veya otomasyon sisteminden okuyun."
    )
    ...
    if submitted:
        if not heat_id:
            st.error("Heat ID / Şarj No zorunludur.")
        else:
            if sim_mode:
                st.warning("Simülasyon modunda yeni girişler dosyaya yazılmıyor (sadece test).")
            else:
...
def show_arc_optimizer_page(sim_mode: bool):
    ...
    if sim_mode:
        ...
        st.markdown(
            ...
            "model önerilerini gösterir."
        )

    data_source = generate_simulation_runtime_data() if sim_mode else runtime_data

    if not data_source:
        st.info("Arc Optimizer için henüz canlı veri yok. Önce 2. sayfadan veri ekleyin.")
        return
    ...
    # Tahmin çizgisi: ilk nokta tam "şimdi"de (gap yok)
    future_points = []
    ...
    # Tahmini döküm noktası (Tap T) – içi dolu nokta
    ...
    # Şimdi dikey çizgisi: son gerçek zaman
    ...
    # Geri sayım ve tahmin metni (grafik üzerinde sağ tarafa)
    now = datetime.now()
    delta_now = predicted_tap_time - now
    remaining_sec = max(0, int(delta_now.total_seconds()))
    rem_min = remaining_sec // 60
    rem_sec = remaining_sec % 60

    label_text = (
        f"AI Tap: {predicted_tap_temp_target:.0f}°C\n"
        f"{predicted_tap_time.strftime('%H:%M')}\n"
        f"T-{rem_min} dk {rem_sec} sn"
    )

    label_df = pd.DataFrame(
        {
            "timestamp_dt": [predicted_tap_time],
            "value": [predicted_tap_temp_target],
            "label": [label_text],
        }
    )
    ...
    label_chart = (
        alt.Chart(label_df)
        .mark_text(align="left", baseline="bottom", dx=8, dy=-8)
        .encode(
            x="timestamp_dt:T",
            y="value:Q",
            text="label:N",
    ...
    st.altair_chart(
        (base_chart + point_chart + now_rule + label_chart).interactive(),
        use_container_width=True,
    )

    delta_min_from_last = (predicted_tap_time - last_time).total_seconds() / 60.0
    st.markdown(
        f"**Tahmini Döküm Anı (AI):** "
        f"{predicted_tap_time.strftime('%Y-%m-%d %H:%M')} "
        f"(son şarjdan itibaren ≈ {delta_min_from_last:.0f} dk, "
        f"şu andan itibaren ≈ {rem_min} dk {rem_sec} sn)"
    )

    # -------------------------------
    # Örnek model önerileri
    # -------------------------------
    ...
    if pd.notna(last.get("kwh_per_t", None)) and avg_kwh_t and not pd.isna(avg_kwh_t) \
        ...
        suggestions.append(
            ...
            "Oksijen debisini ve güç profilini optimize etmeyi düşünün."
        )
    ...
    if pd.notna(last.get("tap_temp_c", None)) and avg_tap_temp and not pd.isna(avg_tap_temp) \
        ...
        suggestions.append(
            "🔥 Tap sıcaklığı düşük. Son aşamada enerji girişini veya bekleme süresini optimize etmek gerekebilir."
        )
